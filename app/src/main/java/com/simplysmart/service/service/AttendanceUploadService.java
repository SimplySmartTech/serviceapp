...
public class AttendanceUploadService extends Service {
        ...
        sendInterfaces.enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                if (response.isSuccessful()) {
                    deleteAttendances(attendanceList);
                }
    ...
    private void deleteAttendances(AttendanceList attendanceList) {
        for (int i = 0; i < attendanceList.getAttendances().size(); i++) {
            new Delete().from(AttendanceTable.class).where("timestamp = ?", attendanceList.getAttendances().get(i).getTime()).execute();
        }
    }
